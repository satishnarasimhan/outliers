# -*- coding: utf-8 -*-
"""
This code is used to automatically download the day wise Futures and Options Bhavcopy as a csv file.
Downloads are restricted to 1 year by the NSE.
Parameters:
d_path = 'Path_to_download_the_extracted_file'
Start_date = The date from which you need the Bhavcopy file in YYYYMONDD format.
End_date = The date from to you need the Bhavcopy file in YYYYMONDD format.
For example, if you need to download each bhavcopy file from 23JUN2020 to 08JUL2020, the input should be as below.
Start_date="2020JUN23"
End_date="2020JUN23"

pip install lxml, requests, pandas, if not already done.
"""
...
for single_date in daterange:
    loop_date = single_date.strftime("%d-%B-%Y")
    file_date = single_date.strftime("%d%m%y")
    ...
    if weekday not in [5,6]:
        ...
        temp_zip_file_url = 'https://www1.nseindia.com/archives/fo/bhav/fo'+file_date+'.zip'
        ## temp_zip_file_url = 'https://www1.nseindia.com/content/fo/fo.zip'
        ##temp_zip_file_url = 'https://nseindia.com/content/historical/DERIVATIVES/'+y+'/'+dmonth[m]+'/fo'+d+dmonth[m]+y+'bhav.csv.zip'
